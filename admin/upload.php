 ...
 * @version : $Id: upload.php 0000 30/03/2009 21:12:57:000 Catzwolf $
 */
...
switch ( $op ) {
	case 'upload':
		...
		if ( !empty( $uploadfile ) ) {
			if ( file_exists( XOOPS_ROOT_PATH . "/${uploadpath}/${uploadfile}" ) ) {
				...
				echo sprintf( _MA_WFC_CHANIMAGEEXIST, $uploadfile );
				xoosla_cp_footer();
				exit();
			}
		...
		if ( $ok ) {
			...
			if ( file_exists( $filetodelete ) ) {
				...
				if ( @unlink( $filetodelete ) ) {
					redirect_header( xoops_getenv( 'PHP_SELF' ), 1, sprintf( _MA_WFP_FILEDELETED, $channelfile ) );
				} else {
					xoops_cp_header();
					echo $menu_handler->render( 5 );
					echo sprintf( _MA_WFP_ERRORDELETEFILE, $channelfile );
					xoosla_cp_footer();
				}
			}
		} else {
			xoops_cp_header();
			$menu_handler->addSubHeader( _MA_WFP_MAINAREA_DELETE_DSC );
			$menu_handler->render( 5 );
			xoops_confirm( array( 'op' => 'delete', 'uploadpath' => $uploadpath, 'channelfile' => $channelfile, 'ok' => 1 ), xoops_getenv( 'PHP_SELF' ), sprintf( _MA_WFP_DYRWTDICONFIRM, $channelfile ), 'Delete' );
		}
		break;
	...
	default:
		xoops_cp_header();
		$menu_handler->addSubHeader( _MA_WFC_UPLOADAREA_DSC );
		$menu_handler->render( 5 );
